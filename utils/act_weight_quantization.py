...
def _quantize_tensor(w, q_group_size=-1, n_bit=4,topk=2):
    ...
    assert w.dim() == 2
    if topk > 0:
        xabs = w.abs()      # 存储x中元素绝对值
        xabs_topk_index = xabs.topk(topk, dim=-1).indices #找到topk元素的索引
        topk_values = torch.gather(w, 1 , xabs_topk_index)#把topk元素提取出来
        w[torch.arange(0, w.size(0), device=w.device)[:, None].expand(-1, topk), xabs_topk_index] = 0 
    ...
    w = torch.clamp(torch.floor(w / scales) + zeros, min_int, max_int)
    # w = w.reshape(org_w_shape).to(torch.uint8)

    if topk > 0:
        row_indices = torch.arange(0, w.size(0), device=w.device).view(-1, 1).expand_as(xabs_topk_index)  # [1024, 2]
        w[row_indices, xabs_topk_index] = topk_values
...
class QLinear(nn.Linear):
    def __init__(self, in_features: int, out_features: int, bias: bool = True,
        ...
        A8Linear.quant_topk = topk

    def forward(self, input: Tensor) -> Tensor:
        qweight, scales, zeros = _quantize_tensor(self.weight, q_group_size=self.group_size, n_bit=self.num_bits,topk=self.topk)
